...
**목표 버전 흐름**: `V_0.1.0`(Baseline, 현재) → `V_0.1.x` → `V_0.1.2` →
`V_0.5.0` → `V_1.0.0`
...
| **V_0.1.2** | Phase 6(Intent Engine) 착수 — 자연어 → 구조화 Intent 파이프라인 최초 구현 |
...
| **완료 조건** | DB 마이그레이션 베이스라인 작성·커밋 완료(TASK-003, 신규 환경 실제 재현 검증은 미수행), CORS allow-list 적용·커밋 완료(TASK-004), HTTPS/HSTS 확정 문서화(미완료), 자동 테스트 도입(**정정 2026-09-14**: Frontend `flutter test` **283 passing**, Backend `npm test` **230 passing / 68 suites**(하네스 커밋 완료, lingon `b5fad88`) — 양쪽 모두 도입 완료, 남은 것은 **CI 정기 실행**(파이프라인 없음)), Privacy Policy/Terms 법률 검토 완료(미완료). |
