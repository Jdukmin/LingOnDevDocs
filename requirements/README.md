...
### Backend/Frontend를 분리된 프롬프트 세션으로 운영할 때 (2026-07-21 추가)

Backend와 Frontend를 서로 다른 세션에 순차적으로 프롬프트해 구현을 맡기는
경우, 한쪽 세션은 **상대편이 실제로 무엇을 했는지 알 수 없다** — 이 상태에서
75%/100%를 매기면 "Backend 완료·Frontend 미연동" 같은 격차가 문서에만
없는 것처럼 보이는 문제가 재발한다. 아래 상한을 지킨다:

| 상황 | 허용 Progress 상한 |
|---|---|
| 한쪽 세션만 완료·자체 검증 | 25% |
| 양쪽 세션 모두 완료, 서로 연동 여부 미확인 | 50% |
| 통합 확인(양쪽 결과를 한 자리에서 대조)까지 마침 | 75% |
| 테스트/실제 실행까지 확인 | 100% |

절차 세부사항은 [../docs/icd/prompt_playbook.md](../docs/icd/prompt_playbook.md)를 따른다.
